    ...
    try:
        ...
        if all_fingerprints.dtype == object:
            ...
            if not fingerprint_data:
                ...
                print("The file structure might be different from expected (e.g., just storing metadata).")
                return

            all_fingerprints = np.array(fingerprint_data)

        if all_fingerprints.ndim != 2:
            print(f"Error: Expected a 2D array (snapshots x features), but got {all_fingerprints.ndim} dimensions.")
            return
        ...
        plt.xlim(0, 15000)  # Zoom in to 15kHz, as most relevant audio data is below this.
        plt.show()
