...
class RadioHEAD(BoxLayout):
    ...
    def analyze_in_background(self):
        ...
        if stability < 0.13:
            ...
            result, color = "TALK/SPORTS", "ff8800"
        else:
    ...
    def analysis_complete(self, result, color, stability):
        self.status.text = f"[color={color}{result} • Stability {stability:.3f}"
    ...
    def show_plots(self):
        ...
        fig.suptitle('radioHEAD • Analysis', color='cyan', fontsize=18, fontweight='bold')
        ...
        # Decode PNG to raw RGBA bytes using PIL
        ...
        popup = Popup(title="radioHEAD • Plots", size_hint=(0.95, 0.95))
    ...
    def save_data(self):
        ...
        np.save(f"radioHEAD_{ts}.npy", np.column_stack((self.energy_profile, self.avg_fft)))
        self.status(Label( f"Saved radiohead_{ts}.npy", color= 'orange'))
