...
class SoundAnalyzer(BoxLayout):
    ...
    def stop_and_analyze(self, instance):
        ...
        try:
            sd.stop()  # Stop the hardware

            # Extract only the portion recorded so far
            ...
            # Prediction logic
            prediction = "MUSIC (Steady)" if stability_score < 0.15 else "TALK/SPORTS (Spikey)"

            # Compute average FFT
            S = np.abs(librosa.stft(full_audio, n_fft=2048, hop_length=hop_length))
            self.avg_fft = np.mean(S, axis=1)

            self.status_label.text = f"Done! Stability: {stability_score:.4f}\n{prediction}"
    ...
    def save_data(self, instance):
        if self.energy_profile is None:
            self.status_label.text = "WARNING: Nothing to save!"
            return
